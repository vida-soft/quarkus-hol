...
If this annotation is not present, our producer woruld not work, as it will try to search for matching annotation that has `@Gold(limit = 2)` annotation for example.
...
----
<.> We use our qualifier annotation to help CDI choose which metho to call.
...
You must be careful not to confuse it with `javax.ws.rs.Produces`/`jakarta.ws.rs.Produces`, as you already know that this annotation is used to describe the return type for JAX-RS, whcih has nothing to do with CDI producers.
...
----
<.> When we want to call a producer, engaged with an anontation, we always use the `Inject` annotation.
